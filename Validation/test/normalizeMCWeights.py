...
def updatePUWeights(hmc, hdata):
    ...
    ratio = []
    for i in xrange(hdata.GetNbinsX()):
        if (hdata.GetBinContent(i) != 0):
            ratio.append(hmc.GetBinContent(i)/hdata.GetBinContent(i))
        else:
            ratio.append(0)

    if (len(ratio) < 100):
        ratio = ratio + [0 for i in xrange(100-len(ratio))]
...
if __name__ == "__main__":  
    parser = OptionParser()
    parser.add_option("-m", "--mc",   default="TnP_mc.root",   help="MC input filename")
    parser.add_option("-d", "--data", default="myHist.root", help="Data input filename")
